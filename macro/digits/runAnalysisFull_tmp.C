...
//    processDigits(runnum,filepath,outputFile);
  ...
  const auto result = pool.Map([](const Parameters &entry) {
          processDigits(entry.runnum,entry.filepathInput,entry.filepathOutput,entry.ptrGRPLHCIFData);
          return 0;}
          , vecParams);
}
...
*/
  ...
  std::set<unsigned int> setChIDsSpectraPerBC = {0,3,27,44};
...
//    arrHistTimeChID_PerBC[chID] = new TH2F(Form("hTimePerBC_ch%i",chID),Form("Time ch%i per BC;BC;Time",chID),sNBC,0,sNBC,400,-200,200);
...
//    listAmpPerChIDPerBC->Add(arrHistAmpChID_PerBC_adc1_noCuts[chID]);
  ...
  std::set<unsigned int> setChIDsTimePerBC{};
  for(const auto &chID: setChIDsSpectraPerBC) {
    if(chID>=sNchannelsAC) {
      std::cout<<"\nWarning! Check channelID set! Cannot find chID "<<chID<<std::endl;
      continue;
