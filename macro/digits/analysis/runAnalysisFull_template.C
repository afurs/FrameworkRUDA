...
void processDigits(unsigned int runnum , const std::string &filepathInput="o2_ft0digits.root",const std::string &filepathOutput="hists.root",const o2::parameters::GRPLHCIFData *ptrGRPLHCIFData=nullptr, bool uploadFromCCDB=true);
...
//    if(runnum>=521553) continue;
    ...
    std::bitset<sNBC> collBC{};
    vecParams.push_back({runnum,filepath,outputFile,getGRPLHCIFData(runnum)});
...
//    processDigits(runnum,filepath,outputFile);
  ...
  const auto result = pool.Map([](const Parameters &entry) {
          processDigits(entry.runnum,entry.filepathInput,entry.filepathOutput,entry.ptrGRPLHCIFData,true);
//          processDigits(entry.runnum,entry.filepathInput,entry.filepathOutput,nullptr,false);

          return 0;}
          , vecParams);
}

void processDigits(unsigned int runnum, const std::string &filepathInput, const std::string &filepathOutput,const o2::parameters::GRPLHCIFData *ptrGRPLHCIFData, bool uploadFromCCDB)
{
  //Load libraries
  gSystem->Load("libboost_filesystem.so");
  gSystem->Load("$RUDA_ROOT/lib/libCommonRUDA.so");
...
//    mapOutput2Store.insert({filepathOutputBuf, listOutput});
  ...
  {
    const std::string filepathOutputBuf = filepathNoExt+std::string{"_AmpA_BC.root"};
  ...
  {
    const std::string filepathOutputBuf = filepathNoExt+std::string{"_AmpC_BC.root"};
  ...
  listOutputTimeA_BC->SetName("output");
  listGarbage->Add(listOutputTimeA_BC);
  {
    const std::string filepathOutputBuf = filepathNoExt+std::string{"_TimeA_BC.root"};
    mapOutput2Store.insert({filepathOutputBuf, listOutputTimeA_BC});
  }

  //Time per BC side C
  TList *listOutputTimeC_BC = new TList();
  listOutputTimeC_BC->SetOwner(true);
  listOutputTimeC_BC->SetName("output");
  listGarbage->Add(listOutputTimeC_BC);
  {
    const std::string filepathOutputBuf = filepathNoExt+std::string{"_TimeC_BC.root"};
    mapOutput2Store.insert({filepathOutputBuf, listOutputTimeC_BC});
  }
