...
void runAnalysisFull(const std::string &pathToSrcFDD = "/data/work/run3/digits/prod_test/run529418",
                     ...
                     const std::string &pathToDst="histsFull_PbPb") {
    ...
    for(const auto &filepath : vecFilepaths) {
      if(filepath.find("_d1_") != string::npos) {
        ...
        if(mapRunnum2CollBC.find(runnum)==mapRunnum2CollBC.end()) {
          ...
          it.first->second.ptrGRPLHCIFData = ptrGRPLHCIFData;
        }
        else {
          it.first->second.ptrGRPLHCIFData = mapRunnum2CollBC.find(runnum)->second;
        }
        index++;
      }
    }
  ...
  const auto result = pool.Map([](const Parameters &entry) {
          processDigits(entry.runnum,entry.filepathInputFT0,entry.filepathOutput,entry.ptrGRPLHCIFData,true);
          return 0;}
          , vecParams);
}
...
*/
          ...
          if(isCollision) {
            ...
            if(collBC.test(arrChID_lastBC[chID])) {
              ...
              if(setChID.find(chID)!=setChID.end() && !(pmBits & (1<<ChannelDataFT0::kNumberADC))) {
