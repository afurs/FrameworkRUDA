...
void runGridTask(AliAnalysisGrid *alienHandler,TString outputFilename){
  ...
  */
  ...
  //alienHandler->SetJobTag("OrigRequirements = member(other.GridPartitions, \"cc7\");");
  
	AliAnalysisManager *mgr = new AliAnalysisManager("Analysis");
  //mgr->SetGlobalStr("OrigRequirements","member(other.GridPartitions, \"cc7\")");
  ...
  gInterpreter->LoadMacro("AliMyTaskEfficiencyT0.cxx+g");
 ...
 // AliAnalysisTaskMyTask *task = reinterpret_cast<AliAnalysisTaskMyTask*>(gInterpreter->ExecuteMacro("AddMyTask.C"));
	...
	AliMyTaskEfficiencyT0 *taskMain = new AliMyTaskEfficiencyT0(taskName);
