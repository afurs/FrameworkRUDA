...
void runAnalysisFull(const std::string &pathToSrcFDD = "/data/work/run3/digits/production_all/d1",
  ...
  const std::size_t nParallelJobs=20;
...
*/
  ...
  */
  ...
  TH2F *hTriggerFDD_BC = new TH2F("hTriggerFDD_BC","Trigger vs BC, FDD;BC;Trigger",sNBC,0,sNBC,mMapTrgNamesFDD.size(),0,mMapTrgNamesFDD.size());
  HistHelper::makeHistBinNamed(hTriggerFDD_BC,mMapTrgNamesFDD,1);
  listOutput->Add(hTriggerFDD_BC);

  TH2F *hTriggerFT0_BC = new TH2F("hTriggerFT0_BC","Trigger vs BC, FT0;BC;Trigger",sNBC,0,sNBC,mMapTrgNamesFT0.size(),0,mMapTrgNamesFT0.size());
  HistHelper::makeHistBinNamed(hTriggerFT0_BC,mMapTrgNamesFT0,1);
  listOutput->Add(hTriggerFT0_BC);

  TH2F *hTriggerFV0_BC = new TH2F("hTriggerFV0_BC","Trigger vs BC, FV0;BC;Trigger",sNBC,0,sNBC,mMapTrgNamesFV0.size(),0,mMapTrgNamesFV0.size());
  HistHelper::makeHistBinNamed(hTriggerFV0_BC,mMapTrgNamesFV0,1);
  listOutput->Add(hTriggerFV0_BC);
  ...
  TH2F *hAmpPerChFDD_adc0_pure = new TH2F("hAmpPerChFDD_adc0_pure","FDD Amplitude per Channel ID (ADC0, good PM bits, abs(time)<100);Channel [ID];Amplitude [ADC]",sNchannels_FDDAC,0,sNchannels_FDDAC,4200,-100,4100);
  listOutput->Add(hAmpPerChFDD_adc0_pure);

  TH2F *hAmpPerChFDD_adc1_pure = new TH2F("hAmpPerChFDD_adc1_pure","FDD Amplitude per Channel ID (ADC1, good PM bits, abs(time)<100);Channel [ID];Amplitude [ADC]",sNchannels_FDDAC,0,sNchannels_FDDAC,4200,-100,4100);
  listOutput->Add(hAmpPerChFDD_adc1_pure);

  TH2F *hAmpPerChFT0_adc0_pure = new TH2F("hAmpPerChFT0_adc0_pure","FT0 Amplitude per Channel ID (ADC0, good PM bits, abs(time)<100);Channel [ID];Amplitude [ADC]",sNchannels_FT0AC,0,sNchannels_FT0AC,4200,-100,4100);
  listOutput->Add(hAmpPerChFT0_adc0_pure);

  TH2F *hAmpPerChFT0_adc1_pure = new TH2F("hAmpPerChFT0_adc1_pure","FT0 Amplitude per Channel ID (ADC1, good PM bits, abs(time)<100);Channel [ID];Amplitude [ADC]",sNchannels_FT0AC,0,sNchannels_FT0AC,4200,-100,4100);
  listOutput->Add(hAmpPerChFT0_adc1_pure);

  TH2F *hAmpPerChFV0_adc0_pure = new TH2F("hAmpPerChFV0_adc0_pure","FV0 Amplitude per Channel ID (ADC0, good PM bits, abs(time)<100);Channel [ID];Amplitude [ADC]",sNchannels_FV0AC,0,sNchannels_FV0AC,4200,-100,4100);
  listOutput->Add(hAmpPerChFV0_adc0_pure);

  TH2F *hAmpPerChFV0_adc1_pure = new TH2F("hAmpPerChFV0_adc1_pure","FV0 Amplitude per Channel ID (ADC1, good PM bits, abs(time)<100);Channel [ID];Amplitude [ADC]",sNchannels_FV0AC,0,sNchannels_FV0AC,4200,-100,4100);
...
*/
  ...
  TH2F *hCollisionTimeVsVertexFDD_noCut = new TH2F("hCollisionTimeVsVertexFDD_noCut","Collision time vs Vertex position, FDD(w/o cuts);Vertex [cm];Collision time [ns]",2000,-100,100,1000,-50,50);
  listOutput->Add(hCollisionTimeVsVertexFDD_noCut);
  ...
  TH2F *hVertex_FT0_FDD = new TH2F("hVertex_FT0_FDD","Vertex position, FT0 vs FDD (w/o cuts);VertexFT0 [cm];VertexFDD [cm]",2000,-100,100,2000,-100,100);
  listOutput->Add(hVertex_FT0_FDD);

  TH2F *hCollisionTime_FT0_FDD = new TH2F("hCollisionTime_FT0_FDD","Collision time, FT0 vs FDD (w/o cuts);Collision time FT0 [ns];Collision time FDD [ns]",1000,-50,50,1000,-50,50);
  listOutput->Add(hCollisionTime_FT0_FDD);
...
//        int nA{0},nC{0};
        for(const auto &channelDataFDD: channelsFDD) {
          ...
          if(((pmBits & pmBitsToCheck) == pmBitsGood)) {
            if(pmBits & numberADC) {
              hAmpPerChFDD_adc1_pure->Fill(chID,time);
            }
            else {
              hAmpPerChFDD_adc0_pure->Fill(chID,time);
            }
          }
        }
...
//        if(nA>0) timeA=timeA/nA;
//        if(nC>0) timeC=timeC/nC;
        ...
        for(const auto &channelDataFT0: channelsFT0) {
          ...
          if(((pmBits & pmBitsToCheck) == pmBitsGood)) {
            if(pmBits & numberADC) {
              hAmpPerChFT0_adc1_pure->Fill(chID,time);
            }
            else {
              hAmpPerChFT0_adc0_pure->Fill(chID,time);
            }
          }
        }
        ...
        for(const auto &channelDataFV0: channelsFV0) {
          ...
          if(((pmBits & pmBitsToCheck) == pmBitsGood)) {
            if(pmBits & numberADC) {
              hAmpPerChFV0_adc1_pure->Fill(chID,time);
            }
            else {
              hAmpPerChFV0_adc0_pure->Fill(chID,time);
            }
          }
        }
...
//        chDataParamsFDD_noCut.print();
        ...
        if(chDataParamsFT0_noCut.mIsReadyAC)  {
          ...
          if(chDataParamsFDD_noCut.mIsReadyAC) {
            hCollisionTime_FT0_FDD->Fill(chDataParamsFT0_noCut.mCollTime,chDataParamsFDD_noCut.mCollTime);
            hVertex_FT0_FDD->Fill(chDataParamsFT0_noCut.mVrtPos,chDataParamsFDD_noCut.mVrtPos);
          }
        }
        if(chDataParamsFDD_noCut.mIsReadyAC) {
        ...
        if(isSpotFT0_1) {
          //chDataParamsFT0_noCut.print();
          if(chDataParamsFDD_noCut.mIsReadyAC)  {
            hCollisionTimeVsVertexFDD_noCut_spotFT0_1->Fill(chDataParamsFDD_noCut.mCollTime,chDataParamsFDD_noCut.mVrtPos);
          }
          if(chDataParamsFV0_noCut.mIsReadyA) {
            hMeanTimeVsSumAmpFV0_noCut_spotFT0_1->Fill(chDataParamsFV0_noCut.mMeanTimeA,chDataParamsFV0_noCut.mAmpSumA);
            //chDataParamsFV0_noCut.print();
          }
        }
      }// entry SyncMap
